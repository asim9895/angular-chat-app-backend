...
exports.send_message = async (req, res) => {
	...
	try {
		let result = await Conversation.find({$or: [
			{
				participants: {
					$elemMatch: {
						sender,
		...
		if(result.length > 0){
			 ...
			 return res.status(200).json({message: "second message" , result: result[0]._id})
		}else{
...
exports.all_chats_messages = async (req ,res) => {
	const {sender , receiver} = req.body
     try {
		 ...
		 let messages = await Message.findOne({conversation: conversation._id})
		 .sort({ createdAt: -1 })
